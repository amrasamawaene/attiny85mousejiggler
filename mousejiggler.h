...
unsigned int UpperCycleTime = 60000; //Maximum Time in milli-seconds between each mouse action  Default: 30000 (30 Seconds), Max 65535ms
...
//Moves mouse 1 pixel in a direction (up/down/left/right) in a square
  ...
  digitalWrite(1, HIGH);
  DigiMouse.moveY(random(100, 600));
  DigiMouse.delay(50);
  digitalWrite(1, LOW); 
  DigiMouse.delay(random(LowerCycleTime, UpperCycleTime));
  
  digitalWrite(1, HIGH);
  DigiMouse.moveX(random(100, 600)); //
  DigiMouse.delay(50);
  digitalWrite(1, LOW); 
  DigiMouse.delay(random(LowerCycleTime, UpperCycleTime));
